...
sudo docker build . -t urpc-v06
urpc - это тег собираемого образа, можно присвоить любой удобный, например: urpc-v07, urpc-test и т.д.
2. Запустить:
sudo docker run --publish 8080:8888 urpc-v06
Команда запустит контейнер и пробросит порт 8888 из контейнера на порт 8080 запускающего контейнер сервера
Разумно настроить автоматический перезапуск контейнера:
sudo docker run --publish 8080:8888 --restart=always -d urpc-v06
