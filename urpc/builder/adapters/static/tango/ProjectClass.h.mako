...
<%!
    ...
    def _get_tango_type_code(type):
        type = type.type if isinstance(type, ast.ArrayType) else type
        ...
        if type in (ast.Integer8s, ast.Integer8u):
            ...
            return "DEV_VOID"
%>\
<%def name="type_code(arg)">Tango::${_get_tango_type_code(arg.type)}</%def>\
...
<%def name="attribute_class_definition(arg)"><%
    is_array = isinstance(arg.type, ast.ArrayType)
