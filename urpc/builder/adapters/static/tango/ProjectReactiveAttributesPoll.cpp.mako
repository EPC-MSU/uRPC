...
void ${device_name(protocol)}::${poller_name(cmd)}() {
    ...
    while (${poller_need_continue_name(cmd)}.load()) {
...
##             Tango::${tango_type(arg.type)} ${arg.name}_old;
##             Tango::${tango_type(arg.type)} ${arg.name}_new;
##             std::memcpy(${buffer_name(cmd.response)}.${arg.name}}, ${arg.name}_old, ${arg.type.size});
##             % else:
##             const Tango::${tango_type(arg.type)} ${arg.name}_old = ${buffer_name(cmd.response)}.${arg.name};
##             Tango::${tango_type(arg.type)} ${arg.name}_new;
##             % endif
##         % endif
##
##     % endfor
    ...
    % for arg in cmd.response.children:
        % if attr_is_reactive(arg):
##             % if isinstance(arg.type, ast.ArrayType):
##                 std::memcpy(${buffer_name(cmd.response)}.${arg.name}}, ${arg.name}_new, ${arg.type.size});
##             % else:
##                 ${arg.name}_new = ${buffer_name(cmd.response)}.${arg.name};
##             % endif
                // Read from buffer
            % if isinstance(arg.type, ast.ArrayType):
                ${attribute_handler_base_name(arg)}.set_value(reinterpret_cast<Tango::${tango_type(arg.type)}>(${buffer_member_var(arg)}), ${len(arg.type)});
            % else:
                ${attribute_handler_base_name(arg)}.set_value(reinterpret_cast<Tango::${tango_type(arg.type)}*>(&${buffer_member_var(arg)}));
            % endif
##                 // Read from buffer
##             % if isinstance(arg.type, ast.ArrayType):
##                 ${attribute_handler_base_name(arg)}.set_value(reinterpret_cast<Tango::${tango_type(arg.type)}>(${buffer_member_var(arg)}), ${len(arg.type)});
...
##         % if attr_is_reactive(arg):
##             % if isinstance(arg.type, ast.ArrayType):
##             // Array ${arg.name} changed
##             % else:
##             // Scalar ${arg.name} changed
##             % endif
##         % endif
##     % endfor
