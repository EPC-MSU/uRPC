...
/** @file ${device_name(protocol)}.h
    ...
    *   4.   make -j3
	*
	* Предварительно должна быть собрана библиотека ${library_header_file(protocol)},
...
public:
	 ...
	 *	@param s 	Device Name
	 */
	${device_name(protocol)}(Tango::DeviceClass *cl, std::string &s);
	/**
	 * Constructs a newly device object.
	 *
	 *	@param cl	Class.
...
public:
    // Note: init_device/delete_device will be called automatically, but not in constructor/destructor
...
<%def name="buffer_measure_time_name(msg)">${buffer_name(msg)}_measure_time</%def>\
<%def name="buffer_measure_time_got_name(msg)">${buffer_name(msg)}_measure_time_got</%def>\
\
...
private:
    ...
    struct timeval ${buffer_measure_time_name(cmd.response)};
    bool ${buffer_measure_time_got_name(cmd.response)};
...
private:
    ...
    struct timeval ${buffer_measure_time_name(getter.request)};
    bool ${buffer_measure_time_got_name(getter.request)};
...
public:
    ...
    static inline steady_time_point steady_now()
    {
        return std::chrono::steady_clock::now();
    }

    static inline sec_duration steady_time_diff(const steady_time_point &t_start, const steady_time_point &t_end = steady_now())
    {
        return std::chrono::duration_cast<std::chrono::duration<double>>(t_end - t_start).count();
    }
private:
    bool get_time(struct timeval &t, bool &got);

    // High-level
    bool init_hardware();
    void deinit_hardware();
    bool try_reinit_hardware();

    // Low-level
    void do_deinit_hardware();

    bool handle_result(result_t result, std::string info = "");

    // Device reconnect algorithm state
    enum DeviceState {
        // Device was not opened
        DEVICE_NOT_OPENED = 0,

        // Device was successfully opened, not error occurred
        DEVICE_OK,

        // Any error occurred, device not opened, need sleep before reopen
        DEVICE_OPEN_FAILED,

        // Device was opened, but get_identity_information failed or protocol not match
        DEVICE_GINF_FAILED,
    ...
    steady_time_point last_device_open_try_time;
    ...
    std::atomic<bool> pollers_need_continue;
    % else:
