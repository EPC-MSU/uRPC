    ...
    def write_descriptions(req):
        for arg in req.args:
            ...
            if arg.name == "reserved":
                write_ru_en_pair("Reserved ({0} bytes)".format(len(arg.type)),
                                 "Зарезервировано ({0} байт)".format(len(arg.type)))
                write_en_en_pair("Reserved [{0}]".format(len(arg.type)))
            else:
                write_ru_en_pair(_get_description(arg), _get_description_ru(arg))
                write_en_en_pair(arg.name)
                write_en_en_pair(type_to_cstr(arg.type)[0])
...
def build(protocol, out):
    ...
    with ZipFile(out, "w", ZIP_DEFLATED) as archive:
        ...
        archive.write(join(_module_path, "Synch.png"), "Synch.png")
