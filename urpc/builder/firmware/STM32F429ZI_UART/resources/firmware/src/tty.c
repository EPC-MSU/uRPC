...
#define TTY_PORT      GPIOB
#define TTY_PORT_CLK  LL_AHB1_GRP1_PERIPH_GPIOB 
#define TTY_RX_PIN    LL_GPIO_PIN_11
#define TTY_RX_MODE   LL_GPIO_MODE_ALTERNATE 
#define TTY_RX_AF     LL_GPIO_AF_7
#define TTY_TX_PIN    LL_GPIO_PIN_10
#define TTY_TX_MODE   LL_GPIO_MODE_ALTERNATE
#define TTY_TX_AF     LL_GPIO_AF_7

#define TTY_UART      USART3
#define TTY_UART_CLK  LL_APB1_GRP1_PERIPH_USART3
#define TTY_UART_IRQ  USART3_IRQn 

#define TTY_DMA       DMA1
#define TTY_DMA_CLK   LL_AHB1_GRP1_PERIPH_DMA1
#define TTY_DMA_CN    LL_DMA_CHANNEL_4
#define TTY_RX_CHAN   LL_DMA_STREAM_1 
#define TTY_RX_IRQ    DMA1_Stream1_IRQn 
#define TTY_TX_CHAN   LL_DMA_STREAM_3 
#define TTY_TX_IRQ    DMA1_Stream3_IRQn 
...
void TTY_Init(void)
{
  ...
  LL_AHB1_GRP1_EnableClock(TTY_PORT_CLK);
  LL_APB1_GRP1_EnableClock(TTY_UART_CLK);
  LL_AHB1_GRP1_EnableClock(TTY_DMA_CLK);
  
  // Structure sounds good does not work
  ...
  GPIO_InitStructure.Alternate  = TTY_RX_AF;
  ...
  GPIO_InitStructure.Alternate  = TTY_TX_AF;
...
//  DMA_InitStructure.FIFOMode                = LL_DMA_FIFOMODE_ENABLE;//may be nesasary
//  DMA_InitStructure.FIFOThreshold           = 
//  DMA_InitStructure.MemBurst
//  DMA_InitStructure.PeriphBurst

  LL_DMA_Init(TTY_DMA, TTY_RX_CHAN, &DMA_InitStructure);

  LL_DMA_EnableStream(TTY_DMA, TTY_RX_CHAN);
...
static void TTY_DMAHandler(void) // a lot of chages is here
{
  if (LL_DMA_IsActiveFlag_TC1(TTY_DMA) && LL_DMA_IsEnabledIT_TC(TTY_DMA, TTY_RX_CHAN))
  {
    LL_DMA_ClearFlag_TC1(TTY_DMA);  // Clear interrupt source

    TimeoutOrInterrupt = true;
  }

  if (LL_DMA_IsActiveFlag_HT1(TTY_DMA) && LL_DMA_IsEnabledIT_HT(TTY_DMA, TTY_RX_CHAN))
  {
    LL_DMA_ClearFlag_HT1(TTY_DMA);  // Clear interrupt source
  ...
  if (LL_DMA_IsActiveFlag_TC3(TTY_DMA) && LL_DMA_IsEnabledIT_TC(TTY_DMA, TTY_TX_CHAN))
  {
    LL_DMA_ClearFlag_TC3(TTY_DMA);  // Clear interrupt source
...
static void DataTransmitting(void)
{
  LL_DMA_DisableStream(TTY_DMA, TTY_TX_CHAN); 
  ...
  LL_DMA_EnableStream(TTY_DMA, TTY_TX_CHAN);
}
