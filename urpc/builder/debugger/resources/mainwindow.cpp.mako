    ...
    ui(new Ui::MainWindow)
{
	...
	// URLs list in ComboBox
	fill_combobox();
	QCommonStyle style;
	ui->refreshComboBox->setIcon(style.standardIcon(QStyle::SP_BrowserReload));
	QObject::connect(ui->refreshComboBox, SIGNAL(clicked(bool)), this, SLOT(fill_combobox()));

    // get device info
    QObject::connect(ui->devInfoButton, SIGNAL(clicked(bool)), this, SLOT(get_device_info_qt()));
...
{
    ${namespaced("close_device")}(&device);
    device = ${namespaced("open_device")}(comedit->currentText().toStdString().c_str());

    if(device == device_undefined)
    {
        ${namespaced("close_device")}(&device);
        logAdd(comedit->currentText()+ " device undefined");
    }
    else
    {
        logAdd(comedit->currentText()+ " successfully connected");
    }
...
{
    ${namespaced("close_device")}(&device);
    logAdd("Disconnect "+comedit->currentText());
}
...
void MainWindow::enumerate()
{
	...
	logAdd(QString::fromStdString("Found " + std::to_string(dev_number) + " devices"));
	for (int i = 0; i < dev_number; ++i)
		logAdd(QString::fromStdString(${namespaced("get_device_info")}(i).port_name));
}
...
void MainWindow::fill_combobox()
{
    ...
    std::string prefix = "com://";
    #endif
	int item_number = this->comedit->count();
	for (int i = item_number-1; i >= 0; --i)
		this->comedit->removeItem(i);
	int dev_number = ${namespaced("enumerate")}();
	for (int i = 0; i < dev_number; ++i)
		this->comedit->addItem(QString::fromStdString(prefix + std::string(${namespaced("get_device_info")}(i).port_name)));
	this->comedit->setEditable(true);
}

//------------------- Qt wrapper function for get_device_info()
void MainWindow::get_device_info_qt()
{
	// beginning of port name
    #ifdef _WIN32
	int init_pos = 8;
    #else
    int init_pos = 6
	...
	int dev_number = ${namespaced("enumerate")}();
	int i;
	std::string dev_name = comedit->currentText().toStdString().substr(init_pos, len_port_name);
	std::stringstream hex_vid, hex_pid;

	for (i = 0; i < dev_number; ++i)
	{
		if (dev_name == std::string(${namespaced("get_device_info")}(i).port_name))
			break;
	}
	device_info dev_info = ${namespaced("get_device_info")}(i);
	logAdd(QString::fromStdString("Port: " + std::string(dev_info.port_name)));
	hex_vid << std::hex << dev_info.vid;
	logAdd(QString::fromStdString("VID: 0x" + hex_vid.str()));
	hex_pid << std::hex << dev_info.pid;
	logAdd(QString::fromStdString("PID: 0x" + hex_pid.str()));
	logAdd(QString::fromStdString("Manufacturer: " + std::string(dev_info.manufacturer)));
	logAdd(QString::fromStdString("Product: " + std::string(dev_info.product)));
	logAdd(QString::fromStdString("Serial number: " + std::string(dev_info.serial_number)));
	logAdd(QString::fromStdString("Usb bus: " + std::to_string(dev_info.usb_bus)));
	logAdd(QString::fromStdString("Usb address: " + std::to_string(dev_info.usb_address)));
}
