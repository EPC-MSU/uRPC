...
#define SP_API //__declspec(dllexport); Really it doesn't matter commented that line or not. But for consistency with what you can see below, I've commented it.  
#else
/* Using the library - need to import DLL symbols */
#define SP_API // __declspec(dllimport); Please note, using dllimport force us to use import libraries which are not static one! So we've commented that to be able to use truly static linking on Windows.
#endif
#else
