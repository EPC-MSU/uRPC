...
def _convert_protocol(xip):
    ...
    def convert_argument(arg):
        ...
        if isinstance(arg, xiast.NormalField):
            return ast.Argument(type_=arg.type_, name=arg.name, description=descr)
            ...
            return ast.Argument(type_=arg.type_, name=arg.name, description=descr, consts=constants)
        elif isinstance(arg, xiast.ReservedField):
            # Assumed that reserved field can be only one in command fields
            return ast.Argument(type_=ast.Array(ast.Integer8u, arg.size), name="reserved", description=descr)
