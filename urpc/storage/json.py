...
#         self.uuid = uuid
# ast.AstNode.register(TempReference)
...
class TypeField(fields.Field):
    ...
    def _from_type(self, type_obj):
        if isinstance(type_obj, ast.ArrayType):
            return self._from_base_type(type_obj.type) + "[{}]".format(len(type_obj))
        else:
    ...
    def _deserialize(self, value, attr, data):
        parsed = re.search("(?P<type>[u]?int[1-9]+?|float)(?:\\[(?P<length>[0-9]+?)\\])?$", data["type"])
...
class FlagConstantSchema(Schema):
    ...
    # @post_dump
    # def encode(self, obj):
    #     return {"uid": obj.uid} if obj in self.context else obj


class ArgumentSchema(Schema):
    name = fields.String()
    type = TypeField()  # noqa: A003
    description = DescriptionField()
